...
def make_kerberos_request(api_endpoint, data, method):
    ...
    if method == "POST":
        ...
        response = requests.post(url, json=data, auth=kerberos_auth, headers={"Content-Type": "application/json"})
    
    else:

        #here we will make out get request with kerberos_auth and needed headers and json data
        response = requests.get(url, json=data, auth=kerberos_auth, headers={"Content-Type": "application/json"})
...
if register_user:
    ...
    if auth_response:
        ...
        ticket = auth_response.get("ticket")
        ...
        verify_ticket_data = {"ticket": ticket}
